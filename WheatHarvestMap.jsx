/* ============================================================================
   ...
   DATA — TODO: replace with wheat programme data
     wheat-boundaries.geo.json  Placeholder rectangles standing in for Punjab
                          and the project district(s). NOT accurate admin
                          boundaries - swap for real state/district polygons.
     wheat-fields.geo.json      A handful of placeholder farmer-field polygons
                          near villages read off real field-photo geotags
                          (Sherpur Kalan, Kot Kapura, Nurpur Bet, Ludhiana).
                          REPLACE with the wheat programme's FieldKhata/KML
                          export - convert Placemark -> Feature and keep these
                          properties: id, farmer, acres, village, villageName,
                          block, awd, crm.
...
// TODO: replace with wheat programme data - real village list, coordinates and block
const VILLAGES = [
  { key: "sherpur-kalan", name: "Sherpur Kalan", lon: 74.840, lat: 30.840, block: "Moga" },
  { key: "kot-kapura",    name: "Kot Kapura",    lon: 74.843, lat: 30.585, block: "Moga" },
  { key: "nurpur-bet",    name: "Nurpur Bet",    lon: 75.710, lat: 30.956, block: "Ludhiana" },
  { key: "ludhiana-rural", name: "Ludhiana rural belt", lon: 75.944, lat: 30.949, block: "Ludhiana" },
];
...
const CAMERA = {
  ...
  punjab:    { bounds: [[73.6, 29.3], [77.2, 32.7]], padding: 56 },
  district:  { bounds: [[74.4, 30.3], [76.2, 31.2]], padding: 56 },
};
const LEVELS = ["india", "punjab", "district", "village"];
const LEVEL_LABEL = { india: "India", punjab: "Punjab", district: "Project district", village: "Village" };
...
function DrillMap({ level, village, selected, onPickState, onPickVillage, onPickField, onHoverField }) {
  ...
  useEffect(() => {
    ...
    m.on("load", () => {
      ...
      /* project district belt */
      m.addSource("project-district", { type: "geojson", data: bd("project-district") });
  ...
  useEffect(() => {
    ...
    m.setPaintProperty("pd-fill", "fill-opacity", showDistrict ? 0.14 : 0);
    m.setPaintProperty("pd-line", "line-opacity", showDistrict ? 1 : 0);
    ...
    body = (
      <motion.div key={p.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
        ...
        <div className="mt-5">
          ...
          <Row k="Irrigation practice" v={p.awd ? "Enrolled & logged" : "Not enrolled"} accent={p.awd ? C.leaf : "rgba(255,255,255,.5)"} />
          <Row k="Residue" v={p.crm ? "Managed - no burning" : "Retained in field"} accent={p.crm ? C.husk : undefined} />
        </div>
        <p className="mt-5" style={{ fontFamily: FONT_DATA, fontSize: 10.5, color: "rgba(255,255,255,.45)", lineHeight: 1.7 }}>
          {/* TODO: replace with wheat programme data - digital tool names, if different from FieldKhata */}
          Boundary captured by the field team and quality-checked before reporting.
        </p>
      </motion.div>
    );
    ...
    body = (
      <motion.div key={v.key} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
        ...
        <div className="mt-5">
          ...
          {/* TODO: replace with wheat programme data - state/district name */}
          <Row k="District" v="Project district, Punjab" />
        </div>
    ...
    body = (
      <motion.div key={level} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
        ...
        <h3 className="mt-4" style={{ color: "#fff", fontWeight: 700, fontSize: "1.75rem", lineHeight: 1.1 }}>
          {/* TODO: replace with wheat programme data */}
          {level === "india" ? "Where the wheat comes from" : level === "punjab" ? "Punjab" : "Project district"}
        </h3>
        <div className="mt-5">
          <Row k="Mapped fields" v={TOTAL_FIELDS} accent={C.leaf} />
          <Row k="Villages" v={`${VILLAGES.length} shown`} />
          <Row k="Blocks" v="TODO: replace with wheat programme data" />
          <Row k="Headline result" v="TODO: replace with wheat programme data" accent={C.leaf} />
        </div>
        <p className="mt-5" style={{ fontSize: 13.5, lineHeight: 1.7, color: "rgba(255,255,255,.7)" }}>
          {level === "india"
            ? "Punjab is highlighted below - click the state to drop into the project district."
            : level === "punjab"
            ? "Click again to zoom into the project district, where enrolled fields were geofenced."
...
/* ----------------------------------------------------------------------------
   SECTION - drop-in location section for the wheat report
---------------------------------------------------------------------------- */
export default function WheatLocationSection() {
  ...
  const legend = level === "village"
    ? [[C.field, "Irrigation-practice field"], [C.leaf, "Hovered"], [C.husk, "Selected"], [C.mute, "Not enrolled"]]
    : null;
      ...
      <div className="mx-auto" style={{ maxWidth: 1180 }}>
        <div className="mb-10 md:mb-14">
          ...
          <p className="mt-5" style={{ color: C.mute, maxWidth: "62ch", lineHeight: 1.65, fontSize: "1.05rem" }}>
            {/* TODO: replace with wheat programme data - geography, village count, digital tool name */}
            The programme ran across a project belt in Punjab. All {TOTAL_FIELDS} enrolled farmer fields (placeholder
            count) were geofenced - click Punjab to drill from the country down to a single plot.
          </p>
        </div>
